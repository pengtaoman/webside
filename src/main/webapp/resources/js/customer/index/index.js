$(function(){
	$(document).ajaxSend( function(event, jqXHR, options){
		if(options.url.indexOf("/inquiry/") != -1 || options.url.indexOf("/facility/") != -1) {
			return;
		}
...
//			 var parent = document.getElementById('exportForm1');
//			 if (parent != null) {
//				 $('exportForm1').remove();
//			 }
			$('#exportForm1').remove();
			$('#exportFormContainer1').remove();
			//将参数传递后台使用伪表单进行导出
			var exportFormContainer = document.createElement('div');
			exportFormContainer.id = 'exportFormContainer1';
			exportFormContainer.className = 'hidden';
			var exportIframe = document.createElement('iframe');
			exportIframe.name = 'exportIframe';
			exportFormContainer.appendChild(exportIframe);
			var exportForm = document.createElement('form');
			exportForm.id = 'exportForm1';
			exportForm.method = 'post';
			exportForm.target = 'exportIframe';
			exportForm.action = sys.rootPath + url;
			...
			if (param != null) {
				gridPagerInput.value = JSON.stringify(param);
			} else {
				gridPagerInput.value = 'gridPager';
			}
...
//			var index;
//			var options = {  
//			        target: "#exportIframe",         //把服务器返回的内容放入id为output1的元素中  
...
//			};
//			$('#exportForm1').submit(function(){   
//				alert("exportForm1");
//	            $(this).ajaxSubmit(options);   
//	            return false;   //防止表单自动提交  
//	        });
		},
		getData : function(nav, jsonData, callback, callError) {
			var index;
        	$.ajax({
                type : "POST",
                ...
                error : function(errorMsg) {
                	layer.close(index);
                	if(callError) {callError(errorMsg);}
                }
            });
        },	
		postJsonData : function(nav, jsonData, callback, callError) {
			var index;
        	$.ajax({
                type : "POST",
		...
		ajaxGet : function(nav, jsonData, callback, callError, callComplete) {
			var index;
        	$.ajax({
                type : "GET",
                url : sys.rootPath + nav,
    ...
    index : {
        initHomePage : function() {
            ...
            webside.common.loadPage("/welcome.html");
                    ...
                    $(this).bind("click", function() {
                        ...
                        if (sn[sn.length - 1] == '/sirona' || sn[sn.length - 1] == '/druid') {
                            ...
                            for (var i = 0; i < sn.length - 1; i++) {
                            	if (i == sn.length - 2) {
                            		breadcrumb += '<li class="active"><a href="javascript:void(0);" onClick="webside.common.loadPage(\''+ sn[i+1] + '\');">'+ sn[i] + '</a></li>';
                                } else {
    ...
    common : {
         ...
         * @param callback 主函数执行完毕后调用的回调函数名称
         */
        delBatchModel : function(nav, callback) {
            var rows = grid.getCheckedRecords();
            if (rows.length >= 1) {
            	var deleteIds = [];
                var idx;
            	for(idx = 0; idx < rows.length; idx++) {
            		if (nav == '/user/deleteBatch.html') {
                        if (rows[idx].role.key == 'administrator') {
                            layer.msg('该用户为超级管理员,不能删除!', {
                                icon : 0
                            });
                            return false;
                    ...
                    deleteIds.push(rows[idx].id);
            	}
                
            	var msg = '确认删除该行数据?'
            	if (deleteIds.length > 1) {
            		msg = '确认批量删除' + deleteIds.length + '行数据吗？';
            	} 
                layer.confirm(msg, {
                    icon : 2,
                    title : '删除提示'
        ...
        sendPostIds : function(nav, callback, filterCB, confirmMsg) {
            var rows = grid.getCheckedRecords();
            if (rows.length > 0) {
                ...
                layer.confirm(confirmMsg, {
                    ...
                    var ids = [];
                     $.each(rows, function(index, value) {
                    	 if(filterCB == null || filterCB(value)) {
                    		 ids.push(this.id);
                    	 }
                     });
    ...
    form : {
        ...
        facility : {
			...
			initFacilityState : function() {
				var facilityStateInit = $('#facilityStateInit').val();
				var facilityStateNameInit = $('#facilityStateNameInit').val();
				var data = [ {
				...
				$('#facilityState').select2({
					...
					allowClear : true,
					ajax : {
						url : sys.rootPath + "/facility/facilityState",
						dataType : 'json'
					}
				});
				$('#facilityState').val(facilityStateInit).trigger('change');
			},
			initIfTest : function() {
				var ifTestInit = $('#ifTestInit').val();
				var ifTestNameInit = $('#ifTestNameInit').val();
				var data = [ {
					id : ifTestInit,
					text : ifTestNameInit
				} ];
				...
				$('#ifTest').val(ifTestInit).trigger('change');
			},
			initFacilityType : function() {
				var facilityTypeInit = $('#facilityTypeInit').val();
				var data = [ {
					id : facilityTypeInit,
					text : facilityTypeInit
				} ];
				...
				$('#facilityType').select2({
					...
					allowClear : true,
					ajax : {
						url : sys.rootPath + "/facility/facilityType/list",
						dataType : 'json'
					}
				})/*.on('change',  function() {
					$("#facilityName").val('').trigger('change');
					$("#model").val('').trigger('change');
					$("#accuracy").val('').trigger('change');
					$("#testRange").val('').trigger('change');
				});*/
				$('#facilityType').val(facilityTypeInit).trigger('change');
			},
			initFacilityName : function() {
				var facilityNameInit = $('#facilityNameInit').val();
				var data = [ {
					id : facilityNameInit,
					text : facilityNameInit
				} ];
				$('#facilityName').select2({
					...
					allowClear : true,
					ajax : {
						url : sys.rootPath + "/facility/facilityName/list",
						dataType : 'json',
						data : function() {
							var query = {
				...
				$('#facilityName').val(facilityNameInit).trigger('change');
			},
			initModel : function() {
				var modelInit = $('#modelInit').val();
				var data = [ {
					id : modelInit,
					text : modelInit
				} ];
				$('#model').select2({
					...
					allowClear : true,
					ajax : {
						url : sys.rootPath + "/facility/model/list",
						dataType : 'json',
						data : function() {
							var query = {
								facilityType : $('#facilityType').val(),
				...
				$('#model').val(modelInit).trigger('change');
			},
			initAccuracy : function() {
				var accuracyInit = $('#accuracyInit').val();
				var data = [ {
					id : accuracyInit,
					text : accuracyInit
				} ];
				$('#accuracy').select2({
					...
					allowClear : true,
					ajax : {
						url : sys.rootPath + "/facility/accuracy/list",
						dataType : 'json',
						data : function() {
							var query = {
								facilityType : $('#facilityType').val(),
				...
				$('#accuracy').val(accuracyInit).trigger('change');
			},
			initTestRange : function() {
				var testRangeInit = $('#testRangeInit').val();
				var data = [ {
					id : testRangeInit,
					text : testRangeInit
				} ];
				$('#testRange').select2({
					...
					allowClear : true,
					ajax : {						
						url : sys.rootPath + "/facility/testRange/list",
						dataType : 'json',
						data : function() {
							var query = {
								facilityType : $('#facilityType').val(),
				...
				$('#testRange').val(testRangeInit).trigger('change');
			},

			validateFacilityForm : function() {
				$('#facilityForm')
						.validate(
								{
									errorElement : 'div',
									...
									submitHandler : function(form) {
										...
										var url = "";
										if (id != undefined && id != '') {
											url = '/facility/edit.html';
										} else {
											url = '/facility/add.html';
										}
										webside.common.commit('facilityForm',
												url, '/facility/listUI.html');
									}
								});
			},
			delModel : function(nav, callback) {
			...
			doRepair : function(nav, callback) {
				var rows = grid.getCheckedRecords();
				if (rows.length == 1) {
					layer.confirm('确认调修吗？', {
						icon : 3,
							...
							.jstree(
									{
										"core" : {
											'data' : {
												"url" : sys.rootPath
														+ "/resource/withoutAuth/resourceTree.html?roleId="
														+ $("#id").val(),
												"dataType" : "json"
											},
											"themes" : {
