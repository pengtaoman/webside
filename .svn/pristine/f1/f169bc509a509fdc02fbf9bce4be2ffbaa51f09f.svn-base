...
$(function() {
    ...
    var data={};
    webside.getData("/sms/unreadcnt.html", data, 
			  function(result) {
    			if(result.success) {
...
function getUnreadSmsCount() {
	var data={};
	webside.ajaxGet("/sms/unreadcnt.html", data, 
		function(result) {
  			if(result.success && result.data != $("#smsUnreadCnt").html()) {
  				$("#smsUnreadCnt").html(result.data);
  			}  
		}, null, 
		function() {
				...
				<ul class="nav ace-nav">
						<li class="green">
							<a data-toggle="dropdown" class="dropdown-toggle" 
								...
								nav-menu="短消息,/sms/listUI.html?id=${userEntity.id }">
								<i class="ace-icon fa fa-bell icon-animated-bell"></i>
								<span id="smsUnreadCnt" class="badge badge-success">0</span>
							</a>
					...
					<li class="light-blue"><a data-toggle="dropdown" href="#"
						...
						<img class="nav-user-photo" src="${ctx}/resources/images/user.jpg"/>
							<span class="user-info"> <small>欢迎</small> <small id="header_username">${userEntity.userName }</small>
						</span> <i class="ace-icon fa fa-caret-down"></i>
					</a>
						<ul class="user-menu dropdown-menu-right dropdown-menu dropdown-yellow dropdown-caret dropdown-close">
							<li><a href="javascript:void(0)" nav-menu="修改密码,/user/passwordUI.html?id=${userEntity.id }&accountName=${userEntity.accountName}"> 
												...
												<c:forEach var="secondChildrens" items="${firstChildrens.children}" varStatus="idx2">
														<li level="level3" class="">
															...
															<c:if test="${fn:length(secondChildrens.children) gt 0 }" >
																<ul class="submenu">
																	<!-- 四级 -->
																	<c:forEach var="thirdChildrens" items="${secondChildrens.children}" varStatus="idx3">
																			<li level="level4" class="">
																				<a href="javascript:void(0)" <c:if test="${fn:length(thirdChildrens.sourceUrl) gt 0 }">nav-menu="${resource.name },${firstChildrens.name },${secondChildrens.name },${thirdChildrens.name },${thirdChildrens.sourceUrl }"</c:if><c:if test="${fn:length(thirdChildrens.children) gt 0 }"> class="dropdown-toggle"</c:if>> 
																					<i class="menu-icon <c:if test="${fn:length(thirdChildrens.icon) gt 0 }">${thirdChildrens.icon } orange</c:if><c:if test="${fn:length(thirdChildrens.icon) le 0 }">fa fa-caret-right</c:if>"></i> ${thirdChildrens.name }
																				</a> 
																				<b class="arrow"></b>
																			</li>
																	</c:forEach>
		...
		<div class="footer">
			<div class="footer-inner">
				...
				<div class="footer-content">
					<span class="bigger-120"> <span class="blue bolder">Web管理平台</span>
