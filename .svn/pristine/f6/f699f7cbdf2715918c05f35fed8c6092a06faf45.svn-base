...
/**
 * Druid连接池的Quartz扩展类
 * <h1 style="color:red">配置quartz的Non-XA数据源,请参考quartz配置文件</h1>
 * quartz在与其他数据库 打交道的时候需要俩种数据源（connection） 一种是需要参与到外围事物的(spring,servlet开发 )等，一种是自己来维护的，
 * 因为当容器启动的时候，quartz框架会自动去其他数据库 查询需要启动的任务，这个时候的操作是不需要参与到外围事物的，它自己管理就可以了。另外一种情况就是当我们添加一个任务 比如 scheduler.schedulejob(job, trigger); 动态添加一个任务，我们往往都是需要关联一些业务数据的。所以在添加任务到其他数据库 中的同时也要保证业务数据同时入库，所以在这种情况下quartz就需要获取spring上下文的链接 connection，这样才能在同一个事物当中
 * @author gaogang
 * 2016年7月16日 下午11:03:27
 *
 */
